...
async function handleRegister(request, env, corsHeaders) {
  ...
  // Parse the request body
  const data = await request.json();
  const { username, password, email, companyName } = data;
  ...
  try {
    ...
    const result = await env.DB.prepare(
      `INSERT INTO users (username, password, email, companyName, created_at) 
       VALUES (?, ?, ?, ?, ?)`
    ).bind(
      username,
      password,
    ...
    await env.DB.prepare(
      `INSERT INTO provider_profiles (user_id, services, locations, insurance, intake, contact)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).bind(
      newUser.id,
      JSON.stringify([]),
    ...
    await env.DB.prepare(
      `INSERT INTO widget_settings (user_id, primary_color, secondary_color, position, greeting, bot_name, show_branding)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      newUser.id,
      '#4F46E5',
  ...
  try {
    ...
    try {
      ...
      if (conversationId) {
        ...
        const newConversation = await env.DB.prepare(
      ...
      ).bind(JSON.stringify(conversation.messages), conversation.id).run();
